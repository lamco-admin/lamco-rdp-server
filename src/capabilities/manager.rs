...
impl CapabilityManager {
    ...
    async fn probe_all() -> Result<Self, InitializationError> {
        ...
        let degradations = Self::collect_degradations(&display, &encoding, &input, &storage, &rendering);
        let blocking_issues = Self::collect_blocking_issues(&display, &encoding, &input, &rendering);
    ...
    pub fn diagnostic_summary(&self) -> String {
        ...
        let can_server = if self.state.minimum_viable.can_operate_server() { "✅" } else { "❌" };
        let can_gui = if self.state.minimum_viable.can_operate_gui() { "✅" } else { "❌" };
        summary.push_str(&format!(
            "│  Server: {} GUI: {}                       │\n",
            can_server, can_gui
        ));
