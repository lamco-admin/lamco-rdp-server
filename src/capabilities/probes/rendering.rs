...
use super::utils::{detect_display_server, detect_virtualization, run_command, DisplayServer, VirtualizationType};
...
impl RenderingProbe {
    ...
    pub async fn probe() -> RenderingCapabilities {
        ...
        info!("Rendering service level: {:?}, recommendation: {:?}", service_level, recommendation);
    ...
    fn check_software_rendering() -> bool {
        ...
        if let Ok(output) = run_command("glxinfo", &["-B"]) {
            if output.contains("llvmpipe") || output.contains("softpipe") || output.contains("swrast") {
                return true;
            }
        }
                ...
                tokio::task::spawn_blocking(|| {
                    ...
                    if let Ok(output) = run_command("lspci", &[]) {
                        ...
                        if (lower.contains("vga") || lower.contains("3d") || lower.contains("display")) {
                            ...
                            if lower.contains("virtio") || lower.contains("qxl")
                               || lower.contains("cirrus") || lower.contains("bochs") {
    ...
    fn determine_recommendation(
        ...
        if let Some(virt) = virtualization {
            if !wgpu_supported || is_virtual_gpu {
                if software_available {
                    ...
                    return (
                        RenderingRecommendation::UseSoftware { reason: reason.clone() },
                        Some(reason),
                    );
                } else {
                    return (
            ...
            if software_available {
                return (
                    RenderingRecommendation::UseSoftware {
                        reason: "GPU available but not compatible with wgpu; using software rendering".into(),
                    },
        ...
        (
            RenderingRecommendation::NoGui {
                ...
                suggestion: "Install mesa-dri-drivers for software rendering or use CLI: lamco-rdp-server".into(),
            },
            None,
        )
    }
