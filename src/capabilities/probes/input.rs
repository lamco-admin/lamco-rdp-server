...
impl InputProbe {
    ...
    pub async fn probe() -> InputCapabilities {
        ...
        match deployment {
            ...
            DeploymentType::Native | DeploymentType::SystemdUser => {
                ...
                if compositor == "mutter" {
                    ...
                    fallback_chain.push(AttemptResult::success("Mutter API", std::time::Duration::ZERO));
                    ...
                    fallback_chain.push(AttemptResult::success("wlr-direct", std::time::Duration::ZERO));
                }
        ...
        info!("Input service level: {:?}, strategy: {:?}", service_level, selected);
    ...
    fn detect_deployment() -> DeploymentType {
        ...
        if let Ok(cgroup) = std::fs::read_to_string("/proc/1/cgroup") {
            if cgroup.contains("docker") || cgroup.contains("podman") || cgroup.contains("containerd") {
                return DeploymentType::Container;
            }
        }
    ...
    fn detect_compositor_type() -> &'static str {
        ...
        if desktop.contains("gnome") {
            "mutter"
        } else if desktop.contains("sway") || desktop.contains("hyprland") || desktop.contains("wlroots") {
            "wlroots"
