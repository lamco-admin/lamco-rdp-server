...
async fn show_capabilities(format: &str) -> Result<()> {
    ...
    if format == "json" {
        output_capabilities_json(&caps, &deployment, &storage_method, &encryption, accessible, &os_release, &kernel_version);
    } else {
...
fn output_capabilities_json(
    ...
    let guaranteed = services.iter().filter(|s| s["level"] == "guaranteed").count();
    let best_effort = services.iter().filter(|s| s["level"] == "best_effort").count();
    let degraded = services.iter().filter(|s| s["level"] == "degraded").count();
    let unavailable = services.iter().filter(|s| s["level"] == "unavailable").count();

    // Build quirks array
    let quirks: Vec<serde_json::Value> = caps.profile.quirks.iter().map(|q| {
        json!({
            "id": format!("{:?}", q),
    ...
    let recommended_codec = if matches!(caps.profile.recommended_buffer_type, lamco_rdp_server::compositor::BufferType::DmaBuf) {
        Some("avc420")
    } else {
        Some("bitmap")
    };
    ...
    let (deployment_str, linger) = match deployment {
        ...
        lamco_rdp_server::session::DeploymentContext::SystemdUser { linger_enabled } => ("systemd-user", Some(*linger_enabled)),
